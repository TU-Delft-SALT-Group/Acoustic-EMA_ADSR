...
stage=10
decode_nj=2
train_set=eng_kprp_V2_train
test_sets="eng_kprp_V2_test_f_v eng_kprp_V2_test_read_f_v eng_kprp_V2_spon_test_f_v eng_kprp_V2_R_word_test_f_v eng_kprp_V2_R_1-5_test_f_v eng_kprp_V2_R_5-10_test_f_v eng_kprp_V2_R_10-15_test_f_v eng_kprp_V2_R_block_test_f_v"
gmm=tri3b
nnet3_affix=
...
train_data_dir=data/${train_set}_sp_fbank_avv
...
if [ $stage -le 14 ]; then
  ...
  input dim=1104 name=input
  
  # Projection layer to reduce the feature dimension to 80, added by yuanyuan
#  linear-component name=projection input=input dim=80
  ...
  linear-component name=projection dim=80
...
#  utils/mkgraph.sh \
#    --self-loop-scale 1.0 data/lang_test_tglarge \
#    $tree_dir $tree_dir/graph_tglarge || exit 1;
fi
...
if [ $stage -le 24 ]; then
  ...
  for data in $test_sets; do
      steps/nnet3/decode.sh \
          ...
          $tree_dir/graph data/${data} ${dir}/decode_${data}
